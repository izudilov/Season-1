...
class ScrollFotoViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        ScrollFotoViewController.userAPI = userPhoto
        
        var loadPhoto = APIPhotos()
        loadPhoto.loadVKPhotos() { [weak self] foto in
  ...
  /*  func loadVKPhotos1(completion: @escaping ([PhotoSize]) -> Void ) {
        ...
        AF.request(url, method: .get, parameters:
                    parameters).responseJSON { response in
                        ...
                        do {
                            let parsedResult: StructAPIPhoto = try JSONDecoder().decode(StructAPIPhoto.self, from: data)
